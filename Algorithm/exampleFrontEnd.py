...
def saveFunction():
    pickle_out = open(filedialog.asksaveasfilename(initialdir="/", title="Select file",
...
def loadFunction(load_window):
    global all_cards
    pickle_in = open(filedialog.askopenfilename(initialdir="/", title="Select file",
                                                filetypes=(("PICKLE files", "*.pickle"), ("all files", "*.*"))), "rb")
...
def creationPopup():
    ...
    finalize_button = Button(creation_window, text="Finalize Card",
                             command=lambda: finalizeCard(card_prompt, card_response, creation_window))
...
def importCards():
    incoming_cards_file = open(filedialog.askopenfilename(initialdir="/", title="Select file",
...
def exportCards():
    outputFile = open(filedialog.asksaveasfilename(initialdir="/", title="Select file",
                                                   filetypes=(("TEXT files", "*.txt"), ("all files", "*.*"))), 'w',
                      newline='')
    card_data = []
    for card in all_cards:
...
try:
    ...
    warning.pack()
    index_card_image = PhotoImage(file=filedialog.askopenfilename(initialdir="/", title="Select file", filetypes=(
